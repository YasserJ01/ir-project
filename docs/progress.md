...
All clustering code is committed. See "Session 2026-06-11 — Mini-Batch K-Means Clustering" below.
...
- **NQ evaluation qrel-filtering bug fixed**: `_compute_metrics()` loaded all 3,452 NQ qrels but evaluated only 200 queries, causing ir_measures to average in 3,252 zero-result queries (17× metric dilution).
...
- **328 Python tests + 18 Vitest passing**, ruff clean. Git HEAD pushed 
...
- **Session 2026-06-10 — RAG G (inline citations) + RAG H (conversation history)**:
  ...
  - Gateway `user_id` fix : default to `"anonymous"` when `None` to prevent 422 errors on embedding/hybrid paths.
